...
class EnhancedReceiptProcessor:
    ...
    def _is_product_name_candidate(self, line: str) -> bool:
        """Перевіряє, чи може рядок бути назвою товару"""
        if not line or len(line) < 3:
            return False
        
        # Виключаємо рядки з цінами
        if re.search(r'\d+[.,]\d{2}', line):
            return False
        
        # Виключаємо службові рядки
        line_lower = line.lower()
        if any(keyword in line_lower for keyword in self.exclude_keywords):
            return False
        
        # Виключаємо рядки, які є просто числами
        if re.match(r'^\d+\s*$', line):
            return False
        
        # Виключаємо рядки з тільки службовими символами
        if re.match(r'^[^\w\u0400-\u04FF]+$', line):
            return False
        
        return True
    
    def _is_product_characteristic(self, line: str) -> bool:
        """Перевіряє, чи є рядок характеристикою товару (об'єм, вага тощо)"""
        if not line or len(line) < 2:
            return False
        
        line_lower = line.lower()
        
        # Характеристики зазвичай містять одиниці вимірювання
        units = ['л', 'мл', 'кг', 'г', 'шт', 'см', 'м', 'б', 'пач', 'уп']
        has_units = any(unit in line_lower for unit in units)
        
        # Або це короткий рядок без цін та службових слів
        is_short = len(line) < 10
        has_no_price = not re.search(r'\d+[.,]\d{2}', line)
        has_no_service_words = not any(keyword in line_lower for keyword in self.exclude_keywords)
        
        return (has_units or (is_short and has_no_price and has_no_service_words))
    
    def _extract_product_name_from_line(self, line: str) -> str:
        """Витягує назву товару з рядка, очищаючи від кодів"""
        # Видаляємо коди товарів з початку (наприклад, "378 Лип-В")
        clean_line = re.sub(r'^\d+\s*[а-яА-Яa-zA-Z][-/]*\s*', '', line)
        
        # Видаляємо коди типу "3К38 er В/"
        clean_line = re.sub(r'^\d*[а-яА-Яa-zA-Z]*\d+\s*[а-яА-Яa-zA-Z]*\s*[/\-]\s*', '', clean_line)
        
        # Якщо після очищення нічого не залишилось, беремо оригінальний рядок
