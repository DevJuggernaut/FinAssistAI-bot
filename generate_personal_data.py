...
def generate_data_for_user(telegram_id, num_transactions=100):
    ...
    try:
        ...
        if not user:
            print(f"❌ Користувач з Telegram ID {telegram_id} не знайдений в базі даних")
            print("Спочатку зареєструйтесь у боті (/start)")
            return
        
        print(f"🎯 Генерація тестових даних для користувача: {user.first_name}")
        print(f"   Telegram ID: {user.telegram_id}")
        
        # Встановлюємо базові налаштування, якщо не встановлені
        if not user.is_setup_completed:
            user.initial_balance = user.initial_balance or 10000.0
            user.monthly_budget = user.monthly_budget or 15000.0
            user.currency = user.currency or 'UAH'
...
def main():
    ...
    # Ваш Telegram ID (знайдений в базі)
    ...
    try:
        num_transactions = input("Кількість транзакцій для генерації (за замовчуванням 100): ").strip()
